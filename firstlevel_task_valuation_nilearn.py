...
UPDATED:
- Binary regressors are created ONLY from the "cond" column (trial_type = cond).
- Ignores the stim/outcome/resp columns in the events file (they are not used).
- Two parametric modulators modeled as separate regressors locked ONLY to 'resp':
    1) value_init           from column "parametric"  -> trial_type = 'resp_x_value_init'
    2) value_model_derived  from column "parametric2" -> trial_type = 'resp_x_value_model_derived'
- No 'stim' regressor in the design matrix; instead, 'stim_on' is a contrast:
  baseline + resp + outcome (+ toolate if present).
- Handles optional 'toolate' condition: contrast auto-adapts if missing.
"""
...
OUTPUT_DIR       = f'{DATA_DIR}/results/nilearn_task_valuation2'
...
RUNS = [
    ...
    # {'session': '2', 'task': 'learning'},  # add more sessions/tasks as needed
]

# ── Parametric Modulators ────────────────────────────────────────────────────
MOD1_COL = 'parametric'    # Value_init
MOD2_COL = 'parametric2'   # Value_Model_Derived
MEAN_CENTER_MODS = True

# ── Contrasts (base; stim_on is built dynamically) ───────────────────────────
BASE_CONTRASTS = {
    'resp':                'resp',
    'outcome':             'outcome',
    'Value_init':          'resp_x_value_init',
    'Value_Model_Derived': 'resp_x_value_model_derived',
    # 'stim_on' added after model fit based on existing columns
}
...
def _bids_paths(sub, ses, task):
    ...
    Events:      {DATA_DIR}/event_files/{sub}/{task}_{sub}_NILEARN_task_valuation_final.csv
    """
    ...
    return {
        ...
        'events':     f"{DATA_DIR}/event_files/{sub}/{task}_{sub}_NILEARN_task_valuation_final.csv",
    }


def process_events(events_path):
    """
    Create nilearn-compatible events DataFrame with:
      - binary condition regressors from `cond`
      - parametric regressors only on resp events:
          * resp_x_value_init           (from MOD1_COL)
          * resp_x_value_model_derived  (from MOD2_COL)
    Ignores stim/outcome/resp columns in the CSV.
    """
    sep = '\t' if str(events_path).endswith('.tsv') else ','
    events = pd.read_csv(events_path, sep=sep, na_values=['', ' ', 'NA', 'NaN'])

    expected = {'onset', 'duration', 'cond', MOD1_COL, MOD2_COL}
    ...
    # Clean strings
    events['cond'] = events['cond'].astype(str).str.strip()
    ...
    # Mean-center each parametric modulator across non-missing entries (global across run)
    if MEAN_CENTER_MODS:
        for col in [MOD1_COL, MOD2_COL]:
            vals = events[col]
            if vals.notna().any():
                m = vals[vals.notna()].mean()
                events.loc[vals.notna(), col] = vals[vals.notna()] - m
    ...
    for _, row in events.iterrows():
        ...
        # Binary regressor for this condition
        ...
        # Parametric regressors only for resp events
        if cond == 'resp':
            if pd.notna(row[MOD1_COL]):
                nilearn_events.append({
                    'onset': onset,
                    'duration': dur,
                    'trial_type': 'resp_x_value_init',
                    'modulation': float(row[MOD1_COL])
                })
            if pd.notna(row[MOD2_COL]):
                nilearn_events.append({
                    'onset': onset,
                    'duration': dur,
                    'trial_type': 'resp_x_value_model_derived',
                    'modulation': float(row[MOD2_COL])
                })
...
def build_contrasts_for_columns(columns):
    """
    Build a contrast dict that is guaranteed to work given available columns.
    stim_on is defined as baseline + resp + outcome (+ toolate if present).
    """
    cols = set(columns)

    contrasts = dict(BASE_CONTRASTS)

    stim_terms = [t for t in ['baseline', 'resp', 'outcome', 'toolate'] if t in cols]
    if stim_terms:
        contrasts['stim_on'] = ' + '.join(stim_terms)

    # Drop any contrasts whose regressor token(s) are missing
    safe_contrasts = {}
    for name, expr in contrasts.items():
        tokens = [tok.strip() for tok in expr.replace('+', ' ').replace('-', ' ').split()]
        if all((t in cols) for t in tokens):
            safe_contrasts[name] = expr
    return safe_contrasts
...
def run_subject(subject_id):
    ...
    # Build contrasts that match the actual fitted design columns (across sessions)
    all_cols = set()
    for dm in glm.design_matrices_:
        all_cols |= set(dm.columns)

    CONTRASTS = build_contrasts_for_columns(all_cols)
    ...
    for contrast_name, contrast_def in CONTRASTS.items():
        try:
            ...
            print(f"    ✓ {contrast_name}: {contrast_def}")

        except Exception as e:
            print(f"    ✗ {contrast_name} ({contrast_def}): {e}")
