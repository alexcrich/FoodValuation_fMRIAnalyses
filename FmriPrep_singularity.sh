...
#
#SBATCH --j foodvaluation_5_21_25 # job name
#SBATCH --array=1 # number of participants as range starting at 1 (i.e., for 5 participants: 1-5)
#SBATCH --time=24:00:00 # HPC will give you this amount of time to run the process. This is usually enough time
#SBATCH -n 1 # how many nodes you are asking. This is running each subject on a different node so 1 is enough
#SBATCH --cpus-per-task=4 # How many CPUs. This is enough cpus no need to change
#SBATCH --mem-per-cpu=16G # How much memory per CPU. This is enough memory no need to change
...
#SBATCH --mail-user=a.rich@yale.edu # replace with your email
...
BASE_DIR="/gpfs/gibbs/pi/levy_ifat/Alex/foodvaluation" # not really used, not sure why is it here
BIDS_DIR="/gpfs/gibbs/pi/levy_ifat/Alex/foodvaluation/foodvaluation_BIDS" # Location of the bids folder.
DERIVS_DIR="derivatives" # the derivatives folder should be inside the bids folder.
WORK_DIR="/home/acr69/palmer_scratch" # enter working directory here - preferably on scratch60.
...
cmd="${SINGULARITY_CMD} ${BIDS_DIR} ${BIDS_DIR}/${DERIVS_DIR} participant --participant-label ${SUBJ[$SLURM_ARRAY_TASK_ID-1]} -w /work/ -vv --omp-nthreads 8 --nthreads 12 --mem_mb 30000 --skip-bids-validation --output-spaces MNI152NLin2009cAsym:res-2 anat fsnative fsaverage5 --cifti-output"
      # --use-aroma"
